    ...
    <div className={`${navBg ? 'bg-blue-950 shadow-md' : ''} transition-all duration-200 h-[12vh] z-[1000] fixed w-full`}>
      <div className="flex justify-between items-center h-full px-10 w-[90%] xl:-[80%] mx-auto">
        ...
        <div className="hidden lg:flex items-center space-x-10">
            { navLinks.map((link) => {
                return (
                    <Link className="relative text-white text-base
                    font-medium w-fit block after:block after:content-[''] after:absolute after:h-[3px] after:bg-yellow-300 after:w-full after:scale-x-0 after:hover:scale-x-100 after:transition duration-300 after:origin-right" key={link.id} href={link.url}>
                        {link.label} 
                    </Link>
                )
            })}
